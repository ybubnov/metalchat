...
make_llama(const metalchat::safetensor_file& tensors, device& gpu, std::size_t nlayers = 16)
{
    ...
    for (std::size_t i = 0; i < nlayers; i++) {
        ...
        llama::feed_forward ff(
            ...
            tensors[layer_name + "feed_forward.w3.weight"].as<T, 2>(alloc), gpu
        );
        ...
        llama::attention attention(
            ...
            tensors[layer_name + "attention.wo.weight"].as<T, 2>(alloc), options, gpu
        );

        nn::rmsnorm attention_norm(
            tensors[layer_name + "attention_norm.weight"].as<T, 1>(alloc), gpu
        );
        nn::rmsnorm ff_norm(tensors[layer_name + "ffn_norm.weight"].as<T, 1>(alloc), gpu);
