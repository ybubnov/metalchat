...
TEST_CASE("Test lfm2", "[lfm][integration]")
{
    auto repo_path = test_fixture_path() / "LiquidAI/LFM2.5-230M";

    hardware_accelerator gpu0;
    filesystem_repository<huggingface::lfm2> repository(repo_path, gpu0);
    ...
    TokenizerTraits::encode(tokenizer, "<|startoftext|>", output);
